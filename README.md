...
提供動画 `OneBe_Process_to_Logo_1920x1080_v2.mp4` を `public/assets/onebe-loading-v2.mp4` として内容を変更せず使用しています（10秒・1920×1080・402,860バイト）。同じタブで最初にサイトを開いたときだけ、無音・インラインで先頭の約7.35秒を再生します。動きが完了した時点で直ちにサイトへ切り替え、末尾の静止時間と終了後のフェード待ちは省きます。動画ファイル自体は10秒の原本を保持しています。1024px以上の画面では動画の表示幅を最大800pxに抑え、ロゴ本体の幅は最大約425pxです。スキップボタンとEscapeで即座に進めます。画面比率が違っても動画全体が入る表示です。
